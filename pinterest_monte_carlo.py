...
Note to self: Monte Carlo is basically just "what if we ran the future 
10,000 times with slightly different assumptions each time?" 
The name comes from the casino, which honestly makes it cooler.
"""
...
# SETUP
# ============================================================
np.random.seed(42)  # keeps results reproducible (learned this the hard way)
N = 10_000
...
# std dev = how much each variable can swing in a given sim
params = {
...
for i in range(N):
    ...
    # Volume factor: more users * more engagement = more ad impressions
    # this is the part that makes it "monte carlo" instead of just a spreadsheet
...
# ============================================================
# note: i didn't know what a tornado chart was before this project
# its basically "if this one variable goes bad, how much does revenue move?"
# you compare the 10th percentile (bad case) vs 90th (good case) for each input
...
# Sort by spread (biggest impact on top)
...
bp = ax.boxplot(region_data, tick_labels=region_labels, patch_artist=True,
